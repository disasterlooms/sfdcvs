({
    ...
    pupdate : function(cmp, event, helper) {
        ...
        var opps = cmp.get("v.lineitems");
        console.log('line items...opps ');
        console.log(opps);
        var i;    
        var z;
        var olines=[]; 
        ...
        for (i = 0; i < opps.length; i++) { 
             for (z = 0; z < opps[i].OpportunityLineItemSchedules.length; z++) { 
                 olines.push(opps[i].OpportunityLineItemSchedules[z]);
             }
         }
        var opplines = cmp.get("v.opportunities[0].OpportunityLineItemSchedules");
        var action = cmp.get("c.updatesch");
        action.setParams({"opps": cmp.get("v.lineitems"),
                          "lines" : olines,
