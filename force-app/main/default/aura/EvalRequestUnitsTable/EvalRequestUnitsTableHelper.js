    ...
    showunits : function(cmp, event, helper){
        ...
        action.setCallback(this, function(data) {
            ...
            if (rslt === "SUCCESS"){
                ...
                var returnUnitsTable = cmp.get("v.returnEval");
                var unitsReturn = [];

                for (var i = 0; i < unitRecords.length; i++){
                    console.log('returned data return '+unitRecords[i].Return_Shipping_Request__c);
                    if(unitRecords[i].Return_Shipping_Request__c&&returnUnitsTable){
                        unitsReturn.push(unitRecords[i]);
                        unitRecords = unitsReturn;
                    }
                }

                  console.log(' below is the selected return units? ');
                  console.log(unitsReturn);
